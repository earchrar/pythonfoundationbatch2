...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f'Welcome! Choose your bus : ',reply_markup=markup)
    return BUS
    ...
    if conn.is_connected():
        ...
        try: 
            ...
            for row in rows:
                ...
                if row['seatno'].endswith('2') or row['seatno'].endswith('4'):
                    display += "\n"

            await update.message.reply_text(f"***{bus} Seat Map:\n\n{display}\n\nChoose your seat number (eg., All) : ")
            return SEAT
        finally:
            cursor.close()
            conn.close()
...
# Temp Save SEAT and insert to DB 
async def getseat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["seatno"] = update.message.text
    ...
    update_sql = '''
        UPDATE seats SET 
            status="occupied",user_id=%s,username=%s,booked_at=%s
