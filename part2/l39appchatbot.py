...
class Chat:
    ...
    def conversations(self,text:str) -> str: 
        ...
        if 'hello' in gettext:
            ...
            return f'{self.name}: The current time is {getnow:strftime("%H:%M:%S")}'
        elif 'how are you' in gettext:
            return f'{self.name}: Great, thanks!'
        elif 'bye' in gettext:
            return f'{self.name}: See you later!'
        else: 
            randomrespone:list[str] = ["I don't understand","what?", "I don't know","Repeat again!"]
            return f'{self.name}: {choice(randomrespone)}'
